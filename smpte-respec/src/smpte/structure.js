...
export const name = "smpte/structure";
...
function makeTOCAtLevel($parent, doc, current, level, conf) {
  ...
  for (var i = 0; i < $secs.length; i++) {
    ...
    /* is it a clause? if not, get the title */
    ...
    /* make an ID if one does not exist */
    
    var id = h.id ? h.id : $sec.makeID(null, "clause_" + secno);
    
    secMap[id] =
      (isIntro ? "" : "<span class='secno'>" + secno + "</span>") +
      (isClause ? "" : " <span class='sec-title'>" + title + "</span>");
  
    /* add to the ToC if not a clause */
